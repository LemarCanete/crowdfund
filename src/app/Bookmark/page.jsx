    ...
    return (
        <main className='container mx-auto p-4'>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div key={project.id} className="bg-white p-4 rounded shadow-md h-120 md:h-96 flex flex-col justify-between">
                    <div className="">
                    ...
                    <div className='flex flex-col gap-2 md:flex-row items-center justify-between w-full mt-2'>
                        <Button className='w-full md:w-auto' variant="secondary" onClick={() => router.push(`/Projects/${project.projectId}`)}>More Details</Button>
                        <Button variant="destructive" className="w-full md:w-auto" onClick={() => handleRemoveBookmark(project.id)}><FaTrash className='text-lg me-2'/> Remove Bookmark</Button>
                    </div>
                </div>
                ))}
            </div>
            ...
            <section className="my-8">
                ...
                {filteredProjects.map((project) => (
                    <div key={project.id} className="bg-white p-4 rounded shadow-md h-120 md:h-96 flex flex-col justify-between">
                        ...
                        <div className='flex flex-col gap-2 md:flex-row items-center justify-between w-full mt-2'>
                            <Button variant="secondary" className='w-full md:w-auto' onClick={() => router.push(`/Projects/${project.id}`)}>More Details</Button>
                            <Button className="w-full md:w-auto" onClick={() => addBookmark(project)}>+ Add Bookmark</Button>
                        </div>
                    </div>
                ))}
                </div>
